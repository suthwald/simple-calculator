...
async def fetch_pr_details_and_files(owner, repo, pr, token):
    """Fetch PR details (title, description, base/head SHA) and changed files."""
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Extract important PR info
        pr_title = pr_data.get("title", "")
        pr_description = pr_data.get("body", "") or ""  # body = PR description
    ...
    return files, base_sha, head_sha, pr_title, pr_description
...
# -----------------------------
# 🔄 Process PR (Now includes PR Title + Description)
# -----------------------------
async def process_pr(owner, repo, pr, installation_id):
    try:
        ...
        # Fetch PR details + files
        files, base_sha, head_sha, pr_title, pr_description = await fetch_pr_details_and_files(
            owner, repo, pr, token
        )
        ...
        # Final structured output with PR description
        output = {
            "pr_number": pr,
        ...
        logger.info(f"PR #{pr} full content (with description) saved to {filename}")
        ...
        await comment_pr(
            ...
            f"Full file contents (before & after) have been saved to `{filename}`\n"
        ...
        if should_auto_approve(structured_files):
            ...
            logger.info(f"PR #{pr} auto-approved")
        else:
