...
decompose_agent = LlmAgent(
    ...
    description="Handles the task related to decompose the user query",
    instruction="""You have to decompose the user query and decides which agent should handle it.
1. If the user query have upc number so you have to return `upc_agent`.
2. If the user query have image url(s) so you have to return `image_sequential_agent`.
3. If the user query **do not have either upc number or image url(s)**, then just say that `I do not have tool to reply this query`
DO NOT GIVE THE RESPONSE OTHER THAN THE PROVIDED RESPONSE""",
...
image_list_agent = LlmAgent(
    name="image_list_agent",
...
image_sequential_agent = SequentialAgent(
    ...
    sub_agents=[image_list_agent, get_image_content_agent],
)
...
3. Return the response from the selected agent or the greeting, as appropriate.""",
    model="gemini-2.5-flash",
    tools=[
        AgentTool(decompose_agent),
        AgentTool(upc_agent),
        AgentTool(image_sequential_agent),
    ],
)
